...
<template>
  <a-modal
    ...
    <a-form
      ...
      <a-form-item label="布局描述:" name="locDesc">
        <a-input
          v-model:value="form.locDesc"
          placeholder="请输入布局描述"
          allow-clear/>
      </a-form-item>
      <a-form-item label="布局位置:" name="locId">
        <a-input-number
          :min="0"
          class="ele-fluid"
          placeholder="请输入布局位置"
          v-model:value="form.locId"/>
      </a-form-item>
      <a-form-item
        label="所属站点:"
        name="itemId">
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate().then(() => {
        this.loading = true;
        this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/system/layoutdesc/edit' : '/system/layoutdesc/add', this.form).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
