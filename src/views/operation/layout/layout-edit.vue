  ...
  <a-modal
    ...
    <a-form
      ...
      <a-form-item 
        ...
        <uploadImage :limit="10" :updDir="updDir" v-model:value="form.image"/>
      </a-form-item>
      <a-form-item
        label="所属站点:"
        name="itemId">
    ...
    data() {
      if (this.data) {
        this.form = Object.assign({}, this.data, {
          layoutDesc: this.data.locDesc + " => " + this.data.locId,
        });
        this.isUpdate = true;
      } else {
        this.form = {};
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate().then(() => {
        this.loading = true;
        this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/system/layout/edit' : '/system/layout/add', this.form).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    /* 查询站点列表 */
    getItemList() {
      this.$http.get('/system/item/getItemList').then(res => {
        if (res.data.code === 0) {
    ...
    handleChange(value) {
      ...
      this.$http.get('/system/layoutdesc/getLayoutDescList/' + value).then(res => {
        if (res.data.code === 0) {
          this.layoutDescList = res.data.data.map(d => {
            d.layoutDesc = d.locDesc + " => " + d.locId;
            return d;
          });
        } else {
          this.$message.error(res.data.msg);
