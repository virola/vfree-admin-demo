...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/system/itemcate/delete/' + [row.id]).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    expandAll() {
      let keys = [];
      this.$util.eachTreeData(this.itemCateList, (d) => {
    ...
    editStatus(checked, row) {
      ...
      this.$http.put('/system/itemcate/status', params).then(res => {
        if (res.data.code === 0) {
