...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/system/link/delete/' + [row.id]).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          const hide = this.$message.loading('请求中..', 0);
          this.$http.delete('/system/link/delete', {
    ...
    editStatus(checked, row) {
      ...
      this.$http.put('/system/link/status', params).then(res => {
        if (res.data.code === 0) {
