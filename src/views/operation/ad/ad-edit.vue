  ...
  <a-modal
    ...
    <a-form
      ...
      <a-form-item 
        ...
        <uploadImage :limit="10" :updDir="updDir" v-model:value="form.cover"/>
      </a-form-item>
      <a-row :gutter="16">
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate().then(() => {
        this.loading = true;
        this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/system/ad/edit' : '/system/ad/add', this.form).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    /* 查询广告位列表 */
    getAdSortList() {
      this.$http.get('/system/adsort/getAdSortList').then(res => {
        if (res.data.code === 0) {
