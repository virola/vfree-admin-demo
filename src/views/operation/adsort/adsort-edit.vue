...
<template>
  <a-modal
    ...
    <a-form
      ...
      <a-form-item label="广告位位置:" name="locId">
        <a-input-number
          :min="0"
          class="ele-fluid"
      ...
      <a-form-item label="所属栏目:" name="cateId">
        <a-tree-select
          allow-clear
          :tree-data="cateList"
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate().then(() => {
        this.loading = true;
        this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/system/adsort/edit' : '/system/adsort/add', this.form).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    /* 查询栏目列表 */
    queryCateList() {
      this.$http.get('/system/itemcate/getCateList').then(res => {
        if (res.data.code === 0) {
