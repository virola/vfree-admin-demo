...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        nickname: [
          {required: true, message: '请输入会员昵称', type: 'string', trigger: 'blur'}
        ],
        gender: [
  ...
  mounted() {
    this.queryMemberLevelList();  // 查询会员等级列表
  },
  methods: {
    /* 保存编辑 */
    save() {
      this.$refs.form.validate().then(() => {
        ...
        this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/member/member/edit' : '/member/member/add', this.form).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    queryMemberLevelList() {
      this.$http.get('/member/memberlevel/getMemberLevelList').then(res => {
        if (res.data.code === 0) {
