      ...
      <ele-pro-table
        ...
        <template #toolbar>
          <a-space>
            ...
            <a-button type="danger" @click="removeBatch" v-if="permission.includes('sys:member:dall')">
              <template #icon>
                <delete-outlined/>
              </template>
              <span>删除</span>
...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/member/member/delete/' + [row.id]).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          const hide = this.$message.loading('请求中..', 0);
          this.$http.delete('/member/member/delete', {
    ...
    editStatus(checked, row) {
      ...
      this.$http.put('/member/member/status', params).then(res => {
        if (res.data.code === 0) {
