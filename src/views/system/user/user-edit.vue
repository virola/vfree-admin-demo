  ...
  <a-modal
    ...
    <a-form
      ...
      <a-form-item 
        ...
        <uploadImage :limit="10" :updDir="updDir" v-model:value="form.avatar"/>
      </a-form-item>
      <a-row :gutter="16">
  ...
  methods: {
    ...
    save() {
      this.$refs.form.validate().then(() => {
        ...
        this.$http[this.isUpdate ? 'put' : 'post'](this.form.id ? '/system/user/edit' : '/system/user/add', this.form).then(res => {
          this.loading = false;
          if (res.data.code === 0) {
    ...
    /* 查询角色列表 */
    queryRoles() {
      this.$http.get('/system/role/getRoleList').then(res => {
        if (res.data.code === 0) {
    ...
    /* 查询职级列表 */
    queryLevelList() {
      this.$http.get('/system/level/getLevelList').then(res => {
        if (res.data.code === 0) {
    ...
    /* 查询岗位列表 */
    queryPositionList() {
      this.$http.get('/system/position/getPositionList').then(res => {
        if (res.data.code === 0) {
    ...
    /* 查询部门列表 */
    queryDeptList() {
      this.$http.get('/system/dept/getDeptList').then(res => {
        if (res.data.code === 0) {
