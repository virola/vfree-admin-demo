  ...
  <div class="ele-body">
    <a-card :bordered="false">
      ...
      <ele-pro-table
        ...
        <template #toolbar>
          <a-space>
            <a-button type="primary" @click="openEdit()" v-if="permission.includes('sys:example3:add')">
              <template #icon>
                <plus-outlined/>
              </template>
              <span>新建</span>
            </a-button>
            <a-button type="danger" @click="removeBatch" v-if="permission.includes('sys:example3:dall')">
              <template #icon>
                <delete-outlined/>
              </template>
              <span>删除</span>
        ...
        <template #action="{ record }">
          <a-space>
            <a @click="openEdit(record)" v-if="permission.includes('sys:example3:edit')">修改</a>
            <a-divider type="vertical"/>
            <a-popconfirm
              title="确定要删除此演示案例表吗？"
              @confirm="remove(record)">
              <a class="ele-text-danger" v-if="permission.includes('sys:example3:delete')">删除</a>
            </a-popconfirm>
          </a-space>
        </template>
      </ele-pro-table>
...
<script>
  ...
  import Example3Edit from './example3-edit';
  ...
  export default {
    ...
    methods: {
      ...
      remove(row) {
        ...
        this.$http.delete('/system/example3/delete/' + [row.id]).then(res => {
          hide();
          if (res.data.code === 0) {
      ...
      removeBatch() {
        ...
        this.$confirm({
          title: '提示',
          content: '确定要删除选中的演示案例表吗?',
          icon: createVNode(ExclamationCircleOutlined),
          maskClosable: true,
          onOk: () => {
            const hide = this.$message.loading('请求中..', 0);
            this.$http.delete('/system/example3/delete', {
      ...
      setStatus(checked, row) {
        ...
        this.$http.put('/system/example3/setStatus', params).then(res => {
          if (res.data.code === 0) {
