      ...
      <ele-pro-table
        ...
        <template #toolbar>
          <a-space>
            ...
            <a-button type="danger" @click="removeBatch" v-if="permission.includes('sys:level:dall')">
              <template #icon>
                <delete-outlined/>
              </template>
              <span>删除</span>
...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/system/level/delete/' + [row.id]).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          const hide = this.$message.loading('请求中..', 0);
          this.$http.delete('/system/level/delete', {
    ...
    editStatus(checked, row) {
      ...
      this.$http.put('/system/level/status', params).then(res => {
        if (res.data.code === 0) {
    ...
    exportData() {
      ...
      this.$http
        .post("/system/level/exportExcel", info)
        .then((res) => {
          let data = res.data;
          if (data.code == 0) {
            // 下载文件
            window.location.href = process.env.VUE_APP_UPLOAD_BASE_URL + data.data;
            this.$message.success(res.data.msg);
          } else {
            this.$message.error(res.data.msg);
          }
