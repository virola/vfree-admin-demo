...
export default {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/system/notice/delete/' + [row.id]).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          const hide = this.$message.loading('请求中..', 0);
          this.$http.delete('/system/notice/delete', {
    ...
    editIsTop(checked, row) {
      ...
      this.$http.put('/system/notice/setIsTop', params).then(res => {
        if (res.data.code === 0) {
          row.isTop = checked ? 1 : 2;
          this.$message.success(res.data.msg);
        } else {
          this.$message.error(res.data.msg);
        }
    ...
    editStatus(checked, row) {
      ...
      this.$http.put('/system/notice/status', params).then(res => {
        if (res.data.code === 0) {
