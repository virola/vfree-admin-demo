...
import SysConfigDataEdit from './sys-config-data-edit';

export default {
  name: 'SysConfigData',
  components: {SysConfigDataEdit},
  computed: {
  ...
  methods: {
    ...
    remove(row) {
      ...
      this.$http.delete('/system/configdata/delete/' + [row.id]).then(res => {
        hide();
        if (res.data.code === 0) {
    ...
    removeBatch() {
      ...
      this.$confirm({
        ...
        onOk: () => {
          const hide = this.$message.loading('请求中..', 0);
          this.$http.delete('/system/configdata/delete', {
    ...
    editStatus(checked, row) {
      ...
      this.$http.put('/system/configdata/status', params).then(res => {
        if (res.data.code === 0) {
